...
def writeSospesi_inPrimaNota(listSospesiNew, filePathnameToWrite):
    ...
    with pd.ExcelWriter(filePathnameToWrite, engine ="openpyxl", mode='a', if_sheet_exists='overlay') as writer:
        for i in range(0, len(listSospesiNew)):
            totSospesi = listSospesiNew[i]
            dateFromSospesi = listSospesiNew[i].date
            ...
            convertStringToDatetime(dataframePrimaNota, DATA)
            ...
            for i in range(0, len(dataframePrimaNota)):
                if(dataframePrimaNota.values[i][DATA] == 'DATA'):
                    try:
                        ...
                        if(isinstance(dataframePrimaNota.values[i+1][DATA], datetime.datetime) or isinstance(dataframePrimaNota.values[i+1][DATA], datetime.date) or isinstance(datetime.datetime.strptime(dataframePrimaNota.values[i+1][DATA], "%d/%m/%Y"), datetime.datetime)):
                            if(isinstance(dataframePrimaNota.values[i+1][DATA], str) and isinstance(datetime.datetime.strptime(dataframePrimaNota.values[i+1][DATA], "%d/%m/%Y"), datetime.datetime)):
                                dateFromPrimaNota = datetime.datetime.strptime(dataframePrimaNota.values[i+1][DATA], "%d/%m/%Y")
                            else:
                                dateFromPrimaNota = dataframePrimaNota.values[i+1][DATA]
